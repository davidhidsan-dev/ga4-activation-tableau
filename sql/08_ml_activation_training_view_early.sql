-- ES
-- Propósito: Construir la vista de entrenamiento ML para predecir activación temprana (add_to_cart_early) con señales tempranas.
-- Objeto: CREATE OR REPLACE VIEW ga4-retention-segmentation.analytics.ml_activation_training_early_v1
...
-- Features:
--   - Flags: search_early, view_item_early, scroll_early, begin_checkout_early, purchase_early
--   - Conteos: sessions_early (distinct ga_session_id), view_item_events_early
-- Label: add_to_cart_early (1 si hay add_to_cart en la ventana)
...
-- EN
-- Purpose: Build ML training view to predict early activation (add_to_cart_early) from early behavior signals.
-- Object: CREATE OR REPLACE VIEW ga4-retention-segmentation.analytics.ml_activation_training_early_v1
...
-- Features:
--   - Flags: search_early, view_item_early, scroll_early, begin_checkout_early, purchase_early
--   - Counts: sessions_early (distinct ga_session_id), view_item_events_early
-- Label: add_to_cart_early (1 if add_to_cart occurs within the window)
...
CREATE OR REPLACE VIEW `ga4-retention-segmentation.analytics.ml_activation_training_early_v1` AS
WITH first_seen AS (
  SELECT
    user_pseudo_id,
