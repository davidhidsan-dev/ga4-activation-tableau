-- ES
-- Propósito: Search -> Product View rate por segmento temprano (segmento definido en ventana temprana D0–D3).
...
-- Importante: el rate de búsqueda es GLOBAL (no se limita a D0–D3); usa todas las sesiones del usuario en scope.
-- Output: segment_early, sessions_with_search, sessions_search_and_view_item, search_to_view_item_rate.

-- EN
-- Purpose: Session-level Search -> Product View rate by early segment (segment defined in early window D0–D3).
...
-- Search metric: session grain (user_pseudo_id + ga_session_id) using boolean flags per session.
-- Important: search rate is GLOBAL (not restricted to D0–D3); uses all sessions for scoped users.
-- Output: segment_early, sessions_with_search, sessions_search_and_view_item, search_to_view_item_rate.
...
-- Early-window events to define the segment (D0–D3 by event_date)
events_early AS (
  SELECT
    e.user_pseudo_id,
    e.event_name,
...
-- GLOBAL sessions: all sessions of scoped users (not limited to early window)
events_scoped_global AS (
  SELECT
    e.user_pseudo_id,
