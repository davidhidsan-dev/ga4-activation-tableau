-- ES
-- Propósito: crear tabla agregada por deciles de propensión (BigQuery ML) para la ventana temprana.
-- Qué hace:
--   1) Puntúa usuarios con ML.PREDICT (regresión logística) para obtener p_activate = P(activación=1)
...
-- EN
-- Purpose: build a propensity-decile summary table (BigQuery ML) for the early window.
-- What it does:
--   1) Scores users with ML.PREDICT (logistic regression) to get p_activate = P(activation=1)
...
CREATE OR REPLACE TABLE `ga4-retention-segmentation.analytics.tableau_propensity_deciles_early_v1` AS
WITH scored AS (
  SELECT
    user_pseudo_id,
    first_date,
    add_to_cart_72h AS activated_early,
    (SELECT prob
...
SELECT
  ...
  AVG(activated_early) AS actual_activation_rate
FROM binned
